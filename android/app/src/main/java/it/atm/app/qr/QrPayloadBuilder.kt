...
object QrPayloadBuilder {
    ...
    private fun buildFormat4(
        ...
        val message = buildByteArray {
            ...
            put(longToBytes(header.uid, 0))
            put(longToBytes(header.systemType.toLong(), 6))
            put(longToBytes(header.systemSubType.toLong(), 6))
            put(longToBytes(header.deviceUID, 0))
            put(longToBytes(header.objectUID, 0))
            put(longToBytes(header.objectType.toLong(), 7))
            put(longToBytes(header.payloadSize.toLong(), 6))
            put(payload)
            putZeros(6)
        }
        return if (sigType > 0) {
