...
import it.atm.app.ui.qrcode.QrCodeViewModelFactory
import it.atm.app.ui.ticket.TicketQrScreen
import it.atm.app.ui.ticket.TicketQrViewModel
import it.atm.app.ui.ticket.TicketQrViewModelFactory
...
fun HomeScreen(
    ...
    loginViewModelFactory: androidx.lifecycle.ViewModelProvider.Factory,
    ...
    if (showAddAccountSheet) {
        val addAccountLoginViewModel: it.atm.app.ui.login.LoginViewModel = androidx.lifecycle.viewmodel.compose.viewModel(
    ...
    // Subscription QR Code dialog
    showQrForIndex?.let { index ->
        val app = (context.applicationContext as AtmApp)
        val qrViewModel: QrCodeViewModel = androidx.lifecycle.viewmodel.compose.viewModel(
            factory = QrCodeViewModelFactory(app),
            key = "qr_$index"
        )
    ...
    showTicketDetail?.let { ticket ->
        val app = (context.applicationContext as AtmApp)
        val ticketQrViewModel: TicketQrViewModel = androidx.lifecycle.viewmodel.compose.viewModel(
            factory = TicketQrViewModelFactory(app),
            key = "ticket_qr_${ticket.ticketId}"
        )
    ...
    // Account switcher full-screen page
